...
def test_item_color_matches_tile_background(tmp_path):
    # The bundle's color is the exact fixed background bg_hex bakes into every
    # tile, so the viewer can reuse it instead of hardcoding the color.
    from pview.images import bg_hex

    out = build(_df(tmp_path), name_col="name", image_col="photo", out_dir=tmp_path / "site")
    data = json.loads((out / "data.json").read_text())
    for item in data["items"]:
        assert item["color"] == bg_hex()
